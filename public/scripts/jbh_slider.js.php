<?php 
    ...
    session_start();
	defined('WEB_PATH') ? NULL : define('WEB_PATH', 'http://'.$_SERVER['HTTP_HOST'].'/99aed.com/');
	$expire = 60 * 60 * 24 * 7;
	header('Pragma: public');
	header('Cache$("label").inFieldLabels();-Control: maxage='.$expire);
                ...
                for (i = 1; i <= $this.data('count'); i++) {
                    ...
                    $link.html(sets.pagination.text.replace('{{page}}', i))
                         .click(function () {
                            ...
                            $.ajax({  
                                type: "POST",
                                url: "<?php //echo WEB_PATH?>ajaxFunctions.php?action=grabInfo", 
                                data: "somedat",
                                success: function(response){			
                                    if (response){
                                        $('#randomDealSlider').after(response);
                                    }
                                } 
                            });
